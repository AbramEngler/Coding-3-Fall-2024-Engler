...
public class DoublyLinkedList<T> {
	...
	public void Sort(boolean ascending) 	//insertion sort
	{
		Node<T> currentNode = this.Header.NextNode; 
	    while (currentNode != null) 
	    {
	        Node<T> nextNode = currentNode.NextNode;
	        Node<T> searchNode = currentNode.PreNode;

	        if (ascending) 
	        {
	            while (searchNode != null && searchNode.compareTo(currentNode) > 0) 
	            {
	                searchNode = searchNode.PreNode;
	            }
	        } else 
	        {
	            while (searchNode != null && searchNode.compareTo(currentNode) < 0) 
